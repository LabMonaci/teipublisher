(:~
    ...
    ODD: /db/apps/labmonaci/resources/odd/monacitest.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/monacitest/epub";
            ...
            typeswitch(.)
                ...
                case element(bibl) return
                    if (parent::listBibl) then
                        html:listItem($config, ., ("tei-bibl1"), if (parent::listBibl) then
    (
        if (editor) then
            html:inline($config, ., ("tei-bibl2"), editor)
        else
            (),
        if (title) then
